...
class DataBmp : public Graphies
{
private:
	GPShader* p_GPShader = nullptr;
public:
	int sort = 0;
	WzlBmpInfo m_BmpInfo = {};	//图片信息头部
	BYTE* p_Data = nullptr;
	float width = 0.0f;
	float height = 0.0f;
protected:
	...
	LPDIRECT3DTEXTURE9 m_Texture1x1;
...
public:
	DataBmp(const char file[],int code);
	~DataBmp();
	void CreateTexture();
	void CreateTexture1x1();
	void CreateTextureToFile(const TCHAR file[]);
	void SetPSShader(GPShader* _psshader);
	void Draw(float x = 0.0f, float y = 0.0f);
	void Draw(float wo, float ho,float x = 0.0f, float y = 0.0f);
};